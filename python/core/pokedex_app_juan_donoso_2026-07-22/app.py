...
def pokemon_nombre(nombre):
    ...
    return pokemon_no_encontrado(
        f'No pudimos encontrar información sobre "{nombre}" en nuestra Pokédex.'
    )
...
def pokemon_id(id):
    ...
    return pokemon_no_encontrado(
        f"No existe un Pokémon con el número {id}."
    )
...
def mostrar_por_nombre(nombre):
    ...
    for p in pokedex:
        if p['nombre'].lower() == nombre.lower():
            return render_template('pokemon.html', pokemon_list=[p], titulo=f"Pokémon: {p['nombre']}")
...
def mostrar_por_id(id):
    for p in pokedex:
        if p['id'] == id:
            return render_template('pokemon.html', pokemon_list=[p], titulo=f"Pokémon: {p['nombre']}")
    return pokemon_no_encontrado(f"No pudimos encontrar información sobre el Pokémon #{id}.")

@app.route('/pokemon/cantidad/<int:cantidad>')
def mostrar_cantidad(cantidad):
    lista = pokedex[:cantidad]
    return render_template('pokemon.html', pokemon_list=lista, titulo=f"Primeros {cantidad} Pokémon")
